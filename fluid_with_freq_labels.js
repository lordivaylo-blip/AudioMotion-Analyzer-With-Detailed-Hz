...
function drawCallback() {
    ...
    const freqs = [];
    for ( let hz = 5; hz <= audioMotion.maxFreq; hz += 5 ) {
        freqs.push(hz);
    }

    freqs.forEach( hz => {
        const x = audioMotion.freqToX(hz);
        ctx.beginPath();
        ctx.moveTo(x, canvas.height - 15 * pixelRatio);
        ctx.lineTo(x, canvas.height);
        ctx.strokeStyle = '#fff8';
        ctx.stroke();
        ctx.fillText(hz.toString(), x, canvas.height - 14 * pixelRatio);
    });
